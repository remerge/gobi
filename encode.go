...
func (enc *Encoder) encodeStruct(b *encBuffer, engine *encEngine, value reflect.Value) {
	...
	for i := 0; i < len(engine.instr); i++ {
		...
		if instr.indir > 0 {
			...
			state.update(instr)
			if !enc.encodeRef(state, field.Addr()) {
				instr.op(nil, state, field)
			}
			continue
		}
...
func (enc *Encoder) encodeArray(b *encBuffer, value reflect.Value, op encOp, elemIndir int, length int, helper encHelper) {
	...
	for i := 0; i < length; i++ {
		...
		if elemIndir > 0 {
			...
			if enc.encodeRef(state, elem.Addr()) {
				continue
			}
		}
		op(nil, state, elem)
...
func (enc *Encoder) encodeRef(state *encoderState, v reflect.Value) bool {
	p := v.Pointer()
	state.encodeUint((uint64(p)))
	remembered := enc.remember[p]
	enc.remember[p] = true
	return remembered
}
...
func (enc *Encoder) encodeReflectValue(state *encoderState, v reflect.Value, op encOp, indir int) {
	if indir > 0 && v.IsValid() && enc.encodeRef(state, v) {
		return
	}
